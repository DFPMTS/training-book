...
虚拟内存（Virtual Memory） 就是解决这个问题的“魔法”。PA3~PA4.2的核心，就是让你亲手实现这种魔法。你将构建SV32页表机制，它就像一个地址翻译官，把程序看到的、各自独立的“虚拟地址”翻译成真正的“物理地址”。

通过这个过程，每个程序都以为自己独占了整个内存空间，从而被安全地隔离开。这不仅带来了安全性，还使得我们可以使用比物理内存更大的地址空间，甚至实现写时复制(COW, Copy On Write)等高级功能。
...
我们此前的NPC严格遵守RV32E指令集，这意味着它的硬件逻辑中，根本不存在处理乘法和除法的单元。然而，当我们在C代码中写下 `a * b` 或 `c / d` 时，程序却能正确执行，仿佛一切都理所当然。答案藏在编译器和运行时环境（AM）里：编译器将这些高级语言操作，链接到了AM中提供的软件实现函数。
...
现在，你不能再简单地调用一个 `*` 或 `/` 运算符，而是需要深入到逻辑门和时序电路，在你的NPC中亲手设计并实现乘法器与除法器。我们建议你首先实现*流水化Radix-4 Booth乘法器*与*恢复余数的除法器*。
